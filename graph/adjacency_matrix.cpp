...
#include<list>
using namespace std;
vector<vector<int>> graph; // vector of lists to represent adjacency list ,vector<list<pair<int,int>>>sath mai weight bhi store karne ke liye
int vertices; // number of vertices in the graph 
void addEdge(int u, int v, bool bi_directional=true) {
    graph[u][v]=1; // Add v to u’s list, graph mai uss idx u pe jho list hai usme v ko add kar rhe hai
    if(bi_directional) {
...
int main() {
    ...
    cin >> vertices;
    graph.resize(vertices, vector<int>(vertices,0)); // Resize the graph to hold the number of vertices
    int edges;
